...
如果一个方法，可以将项目回复到修改之前，当这些紧急任务处理完成后，再将我正写了一半的代码恢复，那该多好呀
...
很多新手会误以为：
“既然 stash 能存东西，那我就一直 stash，不急着整理。”
...
所以：
stash 不是没用，而是“在更好的上下文隔离手段出现后，使用频率会下降”。
...
对应含义：
- `git stash`：先藏起来
- `git stash list`：看看藏了什么
- `git stash pop`：拿出来并删除记录
- `git stash apply`：拿出来但保留记录

## 一个类比帮助你彻底记住

想象你在书桌上写论文：
- commit：装订成正式版本，放进档案柜
- stash：先把桌上的草稿收进抽屉
- branch：开一个新的专题文件夹
- worktree：直接换到另一张桌子继续做另一件事

这个类比里：
- 需要正式保存，就用 commit
- 只是临时腾桌面，就用 stash
- 要做另一条独立任务线，就用 branch
- 要同时开多个任务现场，就用 worktree
